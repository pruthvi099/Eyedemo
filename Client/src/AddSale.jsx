...
const AddSale = () => {
  ...
  useEffect(() => {
    axios.get('http://localhost:3000/scustomers')
      .then(response => {
    ...
    axios.get('http://localhost:3000/sproducts')
      .then(response => {
  ...
  const handleCustomerChange = async (event) => {
    ...
    setCustomerId(id);
    try {
      const response = await axios.get(`http://localhost:3000/scustomers/${id}`);
  ...
  const handleProductChange = async (index, event) => {
    ...
    try {
      const response = await axios.get(`http://localhost:3000/sproducts/${id}`);
