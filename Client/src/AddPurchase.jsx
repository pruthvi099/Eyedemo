...
const AddPurchase = () => {
  ...
  useEffect(() => {
    axios.get('http://localhost:3000/fsuppliers')
      .then(response => {
    ...
    axios.get('http://localhost:3000/fproducts')
      .then(response => {
  ...
  const handleSupplierChange = async (event) => {
    ...
    setSupplierId(id);
    try {
      const response = await axios.get(`http://localhost:3000/fsuppliers/${id}`);
  ...
  const handleProductChange = async (index, event) => {
    ...
    try {
      const response = await axios.get(`http://localhost:3000/fproducts/${id}`);
  ...
  const handleSubmit = (event) => {
    ...
    axios.post('http://localhost:3000/fpurchases', {
