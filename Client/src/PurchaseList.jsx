...
const PurchaseList = () => {
  ...
  const fetchPurchases = async () => {
    try {
      const response = await axios.get('http://localhost:3000/fpurchases');
  ...
  const fetchPurchaseById = async (id) => {
    try {
      const response = await axios.get(`http://localhost:3000/fpurchases/${id}`);
  ...
  return (
    <div className="purchase-container">
      <Link to="/addpurchase" className="add-purchase-link">New Purchase</Link>
